...
function omg_custom_build_prompt() {
  local enabled=${1}
  ...
  if [[ $is_a_git_repo == true ]]; then
    ...
    prompt+=$(omg_enrich_append $has_deletions $omg_has_deletions_symbol "${red_on_white}")

    # ready
    prompt+=$(omg_enrich_append $has_adds $omg_has_adds_symbol "${black_on_white}")
    prompt+=$(omg_enrich_append $has_modifications_cached $omg_has_cached_modifications_symbol "${black_on_white}")
    prompt+=$(omg_enrich_append $has_deletions_cached $omg_has_cached_deletions_symbol "${black_on_white}")
    ...
    if [[ $detached == true ]]; then
      ...
      prompt+=$(omg_enrich_append $detached "(${current_commit_hash:0:7})" "${black_on_red}")
    else
      if [[ $has_upstream == false ]]; then
        prompt+=$(omg_enrich_append true "-- ${omg_not_tracked_branch_symbol}  --  (${current_branch})" "${black_on_red}")
      else
        ...
        if [[ $has_diverged == true ]]; then
          prompt+=$(omg_enrich_append true "-${commits_behind} ${omg_has_diverged_symbol} +${commits_ahead}" "${white_on_red}")
        else
          if [[ $commits_behind -gt 0 ]]; then
            prompt+=$(omg_enrich_append true "-${commits_behind} ${white_on_red}${omg_can_fast_forward_symbol}${black_on_red} --" "${black_on_red}")
          fi

          if [[ $commits_ahead -gt 0 ]]; then
            prompt+=$(omg_enrich_append true "-- ${white_on_red}${omg_should_push_symbol}${black_on_red}  +${commits_ahead}" "${black_on_red}")
          fi

          if [[ $commits_ahead == 0 && $commits_behind == 0 ]]; then
            prompt+=$(omg_enrich_append true " --   -- " "${black_on_red}")
          fi
        fi

        prompt+=$(omg_enrich_append true "(${current_branch} ${type_of_upstream} ${upstream//\/$current_branch/})" "${black_on_red}")
      fi
    fi
